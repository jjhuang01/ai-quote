...
Windsurf 账号管理工具。支持多账号切换、配额统计、本地 MCP Bridge。

---

## 安装

从 VSIX 安装：扩展面板右上角 `···` → **从 VSIX 安装** → 选择 `ai-quote-2.1.4.vsix`。
...
- **配额统计** — 查看各账号日/周配额使用情况
- **MCP Bridge** — 本地 SSE 桥接服务（默认端口 3456）
...
| 设置项                      | 默认值 | 说明                |
| --------------------------- | ------ | ------------------- |
| `infiniteDialog.serverPort` | `3456` | MCP Bridge 监听端口 |
...
**端口冲突**
在设置中将 `infiniteDialog.serverPort` 改为其他端口（如 `3457`），重新加载窗口。
